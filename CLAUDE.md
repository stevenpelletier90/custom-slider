...
**The demo is a one-example-at-a-time catalog, and snippet parity is structural.** Each example is a single `<template data-ex="…">` near the end of `demo/index.html` holding its own `<style>` plus its markup. The stage renders `template.content.cloneNode(true)`; the code panel prints `template.innerHTML`. Same source, two consumers — so the preview and the copy button can never drift. Switching examples destroys the live instances, aborts one `AbortController` shared by that example's page script, and drops the clone (which takes its `<style>` with it), so no example can leak CSS or listeners into the next. Add an example by adding a `<template>`; the sidebar builds itself from `data-group`/`data-title`. Consequences worth knowing:

- Per-example CSS must live **inside** the template, never in the page's `<style>` — chrome CSS there styles only the shell (`.app-*`). Anything an example needs that sits in the shell is invisible in the copied snippet, which is the exact bug this layout was built to kill.
- Examples use `aria-label`, never `aria-labelledby` — the copied snippet does not carry the page's heading.
- Breakpoints are the platform's Bootstrap ladder: **576 / 768 / 992 / 1200**, measured off `chevroletdemo1.dealeron.com`. Don't reintroduce 640/1024.
- Never name a demo wrapper class `bi-*`. Bootstrap Icons is loaded for the arrow-swap example and every one of its classes uses that prefix; `.bi-arrows` is a real icon and painted a stray glyph on the carousel root.
- `demo/index.html` was assembled once by a throwaway script during the 2026-08-27 rework; that script is not in the repo and is not meant to be. The file is normal hand-maintained source now — edit it directly and keep the parity rules above.
...
- Extending behavior belongs in page script, not the engine — listen for `dlc:change` (bubbles from the root) or read the instance at `element._dlCarousel`. The `ENHANCERS` map in `demo/index.html` holds the reference examples (sliding progress-bar thumb, video `<dialog>`, tabs controller, thumb-strip drag). Each takes `(scope, signal)` and must register everything on that signal so switching examples tears it down.
