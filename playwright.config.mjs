...
// a trace retained on failure, and a server the runner starts itself.
//
...
export default defineConfig({
  ...
  use: {
    ...
    // retain-on-failure, not on-first-retry: retries are 0 deliberately (a
    // flaky-tolerant suite is not a gate), so "the first retry" never happens
    // and the trace setting was dead configuration. This keeps a trace for the
    // run that actually failed, which is the one anyone wants (2026-09-15).
